...
d_opt = SGD(learning_rate=0.0001)
# g_opt = Adam(learning_rate=0.001)
...
###########################
# step 3: training
hist = gan.fit(regular_day[:, 0:7], batch_size=batch_size, epochs=epochs)
#hist = gan.fit(regular_day[:, 0:7], batch_size=batch_size, epochs=epochs, callbacks=[GanMonitor(latent_space_dim, scaler)])
...
reduced_data_frame.to_csv(f'./generated_data/generated_regular_day.csv', index=False)
